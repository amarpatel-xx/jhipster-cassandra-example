...
describe('Product routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IProduct returned by find', async () => {
      // GIVEN
    ...
    it('should navigate to 404 when find returns an empty body', async () => {
      ...
      service.find = vitest.fn(() => of(new HttpResponse<IProduct>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 'val-1' };

      // WHEN
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(productResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith('val-1');
