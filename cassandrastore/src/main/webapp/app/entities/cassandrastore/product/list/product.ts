...
export class ProductComponent implements OnInit {
  ...
  protected onResponseSuccess(response: EntityArrayResponseType, append = false): void {
    ...
    if (append) {
      ...
      this.products.set(this.refineData([...current, ...newItems]));
    } else {
      this.products.set(this.refineData(dataFromBody));
    }
    if (this.hasNextPage && this.totalItems !== null) {
      this.hasNextPage = this.products().length < this.totalItems;
    }
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.productService.querySlice(queryObject).pipe(finalize(() => this.isLoading.set(false)));
  }
