...
import { SaathratriEntityDeleteDialogComponent } from '../delete/saathratri-entity-delete-dialog.component';

@Component({
  standalone: true,
  selector: 'jhi-saathratri-entity',
  templateUrl: './saathratri-entity.component.html',
  imports: [
    RouterModule,
    FormsModule,
    SharedModule,
...
export class SaathratriEntityComponent implements OnInit {
  ...
  saathratriEntities?: ISaathratriEntity[];
  isLoading = false;
  ...
  ngOnInit(): void {
    this.subscription = combineLatest([this.activatedRoute.queryParamMap, this.activatedRoute.data])
      .pipe(
        ...
        tap(() => {
          if (!this.saathratriEntities || this.saathratriEntities.length === 0) {
            this.load();
          }
        }),
      )
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.saathratriEntities = this.refineData(dataFromBody);
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.saathratriEntityService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
