...
import { EntityArrayResponseType, SaathratriEntity3Service } from '../service/saathratri-entity-3.service';
import { SaathratriEntity3DeleteDialogComponent } from '../delete/saathratri-entity-3-delete-dialog.component';

@Component({
  standalone: true,
  selector: 'jhi-saathratri-entity-3',
  templateUrl: './saathratri-entity-3.component.html',
  imports: [
    RouterModule,
    FormsModule,
    SharedModule,
...
export class SaathratriEntity3Component implements OnInit {
  ...
  saathratriEntity3s?: ISaathratriEntity3[];
  isLoading = false;
  ...
  // Saathratri: Composite Primary Key Code
  trackCompositeId = (_index: number, item: ISaathratriEntity3Id): ISaathratriEntity3Id =>
    this.saathratriEntity3Service.getSaathratriEntity3Identifier({ compositeId: item });
  ...
  ngOnInit(): void {
    this.subscription = combineLatest([this.activatedRoute.queryParamMap, this.activatedRoute.data])
      .pipe(
        ...
        tap(() => {
          if (!this.saathratriEntity3s || this.saathratriEntity3s.length === 0) {
            this.load();
          }
        }),
      )
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.saathratriEntity3s = this.refineData(dataFromBody);
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.saathratriEntity3Service.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
