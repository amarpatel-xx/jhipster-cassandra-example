...
import { AddOnsAvailableByOrganizationService, EntityArrayResponseType } from '../service/add-ons-available-by-organization.service';
import { AddOnsAvailableByOrganizationDeleteDialogComponent } from '../delete/add-ons-available-by-organization-delete-dialog.component';

@Component({
  standalone: true,
  selector: 'jhi-add-ons-available-by-organization',
  templateUrl: './add-ons-available-by-organization.component.html',
  imports: [
    RouterModule,
    FormsModule,
    SharedModule,
...
export class AddOnsAvailableByOrganizationComponent implements OnInit {
  ...
  addOnsAvailableByOrganizations?: IAddOnsAvailableByOrganization[];
  isLoading = false;
  ...
  // Saathratri: Composite Primary Key Code
  trackCompositeId = (_index: number, item: IAddOnsAvailableByOrganizationId): IAddOnsAvailableByOrganizationId =>
    this.addOnsAvailableByOrganizationService.getAddOnsAvailableByOrganizationIdentifier({ compositeId: item });
  ...
  ngOnInit(): void {
    this.subscription = combineLatest([this.activatedRoute.queryParamMap, this.activatedRoute.data])
      .pipe(
        ...
        tap(() => {
          if (!this.addOnsAvailableByOrganizations || this.addOnsAvailableByOrganizations.length === 0) {
            this.load();
          }
        }),
      )
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.addOnsAvailableByOrganizations = this.refineData(dataFromBody);
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.addOnsAvailableByOrganizationService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
