...
public class TajUserServiceImpl implements TajUserService {
    ...
    @Override
    public Slice<TajUserDTO> findAllSlice(org.springframework.data.domain.Pageable pageable) {
        LOG.debug("Request to get a slice of TajUsers");
        return tajUserRepository.findAll(pageable).map(tajUserMapper::toDto);
    }
}
