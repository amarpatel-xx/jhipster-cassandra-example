...
public class TajUserResource {
    ...
    private static final String ENTITY_NAME = "cassandrablogTajUser";
    ...
    /**
     * {@code GET  /taj-users/slice} : get tajUsers with Cassandra cursor-based pagination.
     *
     ...
     */
    @GetMapping("/slice")
    public ResponseEntity<List<TajUserDTO>> getAllTajUsersSlice(
        @RequestParam(name = "pagingState", required = false) String pagingState,
        @RequestParam(name = "size", defaultValue = "20") int size
    ) {
        LOG.debug("REST request to get a slice of TajUsers, pagingState: {}, size: {}", pagingState, size);
        ...
        Slice<TajUserDTO> slice = tajUserService.findAllSlice(cassandraPageRequest);
        List<TajUserDTO> result = slice.getContent();
