...
public class SaathratriEntity3Resource {
    ...
    /**
     * {@code GET  /saathratri-entity-3-s/slice} : get saathratriEntity3s with Cassandra cursor-based pagination.
     *
     ...
     */
    @GetMapping("/slice")
    public ResponseEntity<List<SaathratriEntity3DTO>> getAllSaathratriEntity3sSlice(
        @RequestParam(name = "pagingState", required = false) String pagingState,
        @RequestParam(name = "size", defaultValue = "20") int size
    ) {
        LOG.debug("REST request to get a slice of SaathratriEntity3s, pagingState: {}, size: {}", pagingState, size);
        ...
        Slice<SaathratriEntity3DTO> slice = saathratriEntity3Service.findAllSlice(cassandraPageRequest);
        List<SaathratriEntity3DTO> result = slice.getContent();
