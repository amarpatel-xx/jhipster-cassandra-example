...
public class TagResource {
    ...
    private static final String ENTITY_NAME = "cassandrablogTag";
    ...
    /**
     * {@code GET  /tags/slice} : get tags with Cassandra cursor-based pagination.
     *
     ...
     */
    @GetMapping("/slice")
    public ResponseEntity<List<TagDTO>> getAllTagsSlice(
        @RequestParam(name = "pagingState", required = false) String pagingState,
        @RequestParam(name = "size", defaultValue = "20") int size
    ) {
        LOG.debug("REST request to get a slice of Tags, pagingState: {}, size: {}", pagingState, size);
        ...
        Slice<TagDTO> slice = tagService.findAllSlice(cassandraPageRequest);
        List<TagDTO> result = slice.getContent();
