...
public class BlogResource {
    ...
    /**
     * {@code GET  /blogs/slice} : get blogs with Cassandra cursor-based pagination.
     *
     ...
     */
    @GetMapping("/slice")
    public ResponseEntity<List<BlogDTO>> getAllBlogsSlice(
        @RequestParam(name = "pagingState", required = false) String pagingState,
        @RequestParam(name = "size", defaultValue = "20") int size
    ) {
        LOG.debug("REST request to get a slice of Blogs, pagingState: {}, size: {}", pagingState, size);
        ...
        Slice<BlogDTO> slice = blogService.findAllSlice(cassandraPageRequest);
        List<BlogDTO> result = slice.getContent();
