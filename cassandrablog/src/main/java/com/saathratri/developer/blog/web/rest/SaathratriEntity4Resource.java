...
public class SaathratriEntity4Resource {
    ...
    private static final String ENTITY_NAME = "cassandrablogSaathratriEntity4";
    ...
    /**
     * {@code GET  /saathratri-entity-4-s/slice} : get saathratriEntity4s with Cassandra cursor-based pagination.
     *
     ...
     */
    @GetMapping("/slice")
    public ResponseEntity<List<SaathratriEntity4DTO>> getAllSaathratriEntity4sSlice(
        @RequestParam(name = "pagingState", required = false) String pagingState,
        @RequestParam(name = "size", defaultValue = "20") int size
    ) {
        LOG.debug("REST request to get a slice of SaathratriEntity4s, pagingState: {}, size: {}", pagingState, size);
        ...
        Slice<SaathratriEntity4DTO> slice = saathratriEntity4Service.findAllSlice(cassandraPageRequest);
        List<SaathratriEntity4DTO> result = slice.getContent();
    ...
    public ResponseEntity<List<SaathratriEntity4DTO>> findAllByCompositeIdOrganizationIdPageable(
        @RequestParam(name = "organizationId", required = true) final UUID organizationId,
        @RequestParam(name = "pagingState", required = false) String pagingState,
        @RequestParam(name = "size", defaultValue = "20") int size
    ) {
        LOG.debug(
            "REST request to get paginated SaathratriEntity4s with parameters organizationId: {}, pagingState: {}, size: {}",
            organizationId,
            pagingState,
            size
        );
        ...
        if (pagingState == null || pagingState.isEmpty()) {
            ...
            try {
                ...
                LOG.error("Invalid paging state for SaathratriEntity4s", e);
                cassandraPageRequest = CassandraPageRequest.first(size);
            }
        }

        Slice<SaathratriEntity4DTO> slice = saathratriEntity4Service.findAllByCompositeIdOrganizationIdPageable(
            organizationId,
        ...
        if (hasNext && nextPagingState == null) {
            try {
                ...
                LOG.warn("Unable to resolve next paging state for SaathratriEntity4s", e);
            }
        }
