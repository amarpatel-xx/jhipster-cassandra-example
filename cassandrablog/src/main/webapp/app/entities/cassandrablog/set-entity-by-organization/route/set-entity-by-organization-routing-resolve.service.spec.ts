...
describe('SetEntityByOrganization routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ISetEntityByOrganization returned by find', async () => {
      // GIVEN
    ...
    it('should navigate to 404 when find returns an empty body', async () => {
      ...
      service.find = vitest.fn(() => of(new HttpResponse<ISetEntityByOrganization>({ body: null })));
      mockActivatedRouteSnapshot.params = { organizationId: 'val-1' };

      // WHEN
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(setEntityByOrganizationResolve(mockActivatedRouteSnapshot))).rejects.toThrowError(
