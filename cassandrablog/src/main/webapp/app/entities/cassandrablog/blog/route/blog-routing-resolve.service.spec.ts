...
describe('Blog routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IBlog returned by find', async () => {
      // GIVEN
      service.find = vitest.fn(category => of({ category }));
        ...
        TestBed.runInInjectionContext(() => {
          blogResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              ...
              expect(result).toEqual({ category: 'ABC' });
              resolve();
            },
          });
        });
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(blogResolve(mockActivatedRouteSnapshot))).rejects.toThrowError('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith('ABC');
