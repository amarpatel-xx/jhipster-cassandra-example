...
import { TajUserDeleteDialogComponent } from '../delete/taj-user-delete-dialog';
...
export class TajUserComponent implements OnInit {
  ...
  readonly tajUsers = signal<ITajUser[]>([]);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType, append = false): void {
    ...
    if (append) {
      ...
      this.tajUsers.set(this.refineData([...current, ...newItems]));
    } else {
      this.tajUsers.set(this.refineData(dataFromBody));
    }
    if (this.hasNextPage && this.totalItems !== null) {
      this.hasNextPage = this.tajUsers().length < this.totalItems;
    }
  }
  ...
  private getEntityKey(item: ITajUser): string {
    return String(this.tajUserService.getTajUserIdentifier(item));
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.tajUserService.querySlice(queryObject).pipe(finalize(() => this.isLoading.set(false)));
  }
