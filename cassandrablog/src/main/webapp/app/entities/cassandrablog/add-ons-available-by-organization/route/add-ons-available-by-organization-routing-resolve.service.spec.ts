...
describe('AddOnsAvailableByOrganization routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IAddOnsAvailableByOrganization returned by find', async () => {
      // GIVEN
    ...
    it('should navigate to 404 when find returns an empty body', async () => {
      ...
      service.find = vitest.fn(() => of(new HttpResponse<IAddOnsAvailableByOrganization>({ body: null })));
      mockActivatedRouteSnapshot.params = { organizationId: 'val-1', entityType: 'val-2', entityId: 'val-3', addOnId: 'val-4' };

      // WHEN
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(addOnsAvailableByOrganizationResolve(mockActivatedRouteSnapshot))).rejects.toThrowError(
