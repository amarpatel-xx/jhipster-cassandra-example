...
@Injectable()
export class SaathratriEntitiesService {
  readonly saathratriEntitiesParams = signal<
    Record<string, string | number | boolean | readonly (string | number | boolean)[]> | undefined
  >(undefined);
  readonly saathratriEntitiesResource = httpResource<ISaathratriEntity[]>(() => {
    const params = this.saathratriEntitiesParams();
    if (!params) {
      return undefined;
    }
  ...
  /**
   * This signal holds the list of saathratriEntity that have been fetched.
   */
  readonly saathratriEntities = computed(() => (this.saathratriEntitiesResource.hasValue() ? this.saathratriEntitiesResource.value() : []));
  protected readonly applicationConfigService = inject(ApplicationConfigService);
  protected readonly resourceUrl = this.applicationConfigService.getEndpointFor('api/saathratri-entities', 'cassandrablog');
}

@Injectable({ providedIn: 'root' })
export class SaathratriEntityService extends SaathratriEntitiesService {
