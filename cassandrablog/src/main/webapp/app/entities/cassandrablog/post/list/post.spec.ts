...
describe('Post Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ createdDate: 21634 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.posts()[0]).toEqual(expect.objectContaining({ createdDate: 21634 }));
  });

  describe('trackCreatedDate', () => {
    it('should forward to postService', () => {
      const entity = { createdDate: 21634 };
      vitest.spyOn(service, 'getPostIdentifier');
      const createdDate = comp.trackCreatedDate(entity);
      expect(service.getPostIdentifier).toHaveBeenCalledWith(entity);
      expect(createdDate).toBe(entity.createdDate);
    });
  });
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.postsParams()).toMatchObject(expect.objectContaining({ sort: ['createdDate,desc'] }));
  });
