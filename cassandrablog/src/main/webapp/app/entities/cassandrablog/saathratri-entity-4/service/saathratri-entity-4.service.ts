...
@Injectable()
export class SaathratriEntity4sService {
  readonly saathratriEntity4sParams = signal<
    Record<string, string | number | boolean | readonly (string | number | boolean)[]> | undefined
  >(undefined);
  readonly saathratriEntity4sResource = httpResource<ISaathratriEntity4[]>(() => {
    const params = this.saathratriEntity4sParams();
    if (!params) {
      return undefined;
    }
  ...
  /**
   * This signal holds the list of saathratriEntity4 that have been fetched.
   */
  readonly saathratriEntity4s = computed(() => (this.saathratriEntity4sResource.hasValue() ? this.saathratriEntity4sResource.value() : []));
  protected readonly applicationConfigService = inject(ApplicationConfigService);
  protected readonly resourceUrl = this.applicationConfigService.getEndpointFor('api/saathratri-entity-4-s', 'cassandrablog');
}

@Injectable({ providedIn: 'root' })
export class SaathratriEntity4Service extends SaathratriEntity4sService {
