...
describe('SaathratriEntity2 e2e test', () => {
  ...
  beforeEach(() => {
    cy.intercept('GET', /^\/services\/cassandrablog\/api\/saathratri-entity-2-s\b/).as('entitiesRequest');
  ...
  afterEach(() => {
    if (saathratriEntity2) {
      cy.authenticatedRequest({
        method: 'DELETE',
        url: `/services/cassandrablog/api/saathratri-entity-2-s/${saathratriEntity2.compositeId.entityTypeId}/${saathratriEntity2.compositeId.yearOfDateAdded}/${saathratriEntity2.compositeId.arrivalDate}/${saathratriEntity2.compositeId.blogId}`,
      }).then(() => {
  ...
  describe('SaathratriEntity2 page', () => {
    it('should have translated page title', () => {
      ...
      cy.getEntityHeading('SaathratriEntity2').should('not.contain', 'cassandrablogApp.cassandrablogSaathratriEntity2.home.title');
    });
      ...
      beforeEach(() => {
        cy.authenticatedRequest({
          ...
          cy.intercept(
            {
              method: 'GET',
              url: /^\/services\/cassandrablog\/api\/saathratri-entity-2-s\b/,
              times: 1,
            },
            {
              statusCode: 200,
  ...
  it('should toggle the Cassandra search form', () => {
    cy.visit('/');
    cy.clickOnEntityMenuItem(saathratriEntity2PageUrl.substring(1));
