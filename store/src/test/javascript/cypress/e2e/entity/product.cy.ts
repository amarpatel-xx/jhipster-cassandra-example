...
describe('Product e2e test', () => {
  ...
  const productSample = { title: 'clearly ew ah', price: 8238.31, addedDate: 8897 };
  ...
  describe('new Product page', () => {
    ...
    it('should create an instance of Product', () => {
      cy.get(`[data-cy="title"]`).type('unless bah boohoo');
      cy.get(`[data-cy="title"]`).should('have.value', 'unless bah boohoo');

      cy.get(`[data-cy="price"]`).type('2269.04');
      cy.get(`[data-cy="price"]`).should('have.value', '2269.04');

      cy.setFieldImageAsBytesOfEntity('image', 'integration-test.png', 'image/png');

      cy.get(`[data-cy="addedDate"]`).type('29579');
      cy.get(`[data-cy="addedDate"]`).should('have.value', '29579');
